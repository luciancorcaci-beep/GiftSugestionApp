...
> **Reconfirmed 2026-09-24**: re-checked against the current working tree. No changes to `src/`, `scripts/`, or `package.json` since commit `e201ccc` (Story 4.1) — the exact commit this document already reflects (162-entry catalog, 12 relationship options, all current). No re-inspection was needed.
>
> **Redone from scratch, 2026-09-24** (user explicitly requested a fresh full re-inspection, not just a reconfirmation): independently re-scanned root directory structure, `package.json` (deps/scripts unchanged), `.env.example` (unchanged, still just `TRUSTED_PRODUCT_DOMAINS`), and confirmed no `next.config.js` exists (still using Next.js defaults). `.github/workflows/{ci.yml,codeql.yml}` and `.github/dependabot.yml` re-verified present and unchanged (an initial grep pattern accidentally excluded `.github` from the directory listing — confirmed separately, no actual gap). Zero drift found — every fact in this document still holds.
...
| Cross-cutting (`lib`) | `errors.ts`, `logger.ts`, `rateLimiter.ts`, `requestBody.ts`, `productUrl.ts`, `health-handler.ts` | Typed error hierarchy + HTTP mapping, structured logger, fixed-window rate limiter, bounded JSON body reader, trusted-URL allowlist, health-check handler | — |
...
- ~~Dead code: `ConcurrencyLimiter` in `src/lib/rateLimiter.ts` appears to be an unused leftover from the retired AI-provider adapter~~ — **Resolved 2026-09-24**: removed, along with its dedicated test block.
...
- ~~Confirm whether `ConcurrencyLimiter` should be deleted along with its now-orphaned purpose.~~ **Done 2026-09-24.**
